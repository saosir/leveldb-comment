...
class VersionSet::Builder {
 ...
 public:
  ...
  ~Builder() {
    // 清理每个level的added_files，引用计数减1，为0删除
    for (int level = 0; level < config::kNumLevels; level++) {
...
void VersionSet::AppendVersion(Version* v) {
  ...
  // 设置为当前version，version2 = version1 + version_edit
  current_ = v;
  v->Ref();
...
Status VersionSet::Recover(bool *save_manifest) {
  ...
  {
    // 读取 manifest 里面的version_edit，并将version_edit应用到 Builder 中
  ...
  if (s.ok()) {
    // version2 = version1 + version_edit
    // 将manifest中的version_edit应用到当前version
    Version* v = new Version(this);
    builder.SaveTo(v);
...
void VersionSet::Finalize(Version* v) {
  ...
  for (int level = 0; level < config::kNumLevels-1; level++) {
    ...
    if (level == 0) {
      ...
      // level0 计算 score 方式为 n/l n:level0文件数 l:触发level0 compaction 的limit
      score = v->files_[level].size() /
          static_cast<double>(config::kL0_CompactionTrigger);
    } else {
      // Compute the ratio of current size to size limit.
      // 整层level文件大小除以当前level最大量限制
